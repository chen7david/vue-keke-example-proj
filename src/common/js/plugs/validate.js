...
const REGEXP = {
    ...
    PASSWORD: /^(?=.*[a-zA-Z~!@#$%^&*()_+`\-={}[\]:";'<>?,./])(?=.*\d).{6,20}$/, // 字符+数字[特殊字符]
    INTEGER: /^\d+$/,
    ...
    CHINESE_NAME: /^[\u4e00-\u9fa5a-zA-Z/._]{1,30}$/i,
    ENGLISH_NAME: /^[a-z][a-z ]+(\s*\/\s*[a-z ]+)?$/i,
    // ENGLISH_NAME: /^[a-z][a-z ]+(\s*\/\s*[a-z ]+)?$/i,
    USER_NAME: /^[a-z0-9A-Z._]+$/,
    // USER_NAME: /^(?![0-9]+$)(?![a-zA-Z]+$)(?![_]+$)[0-9A-Za-z_]+$/,
    STOCKNO: /^[a-z0-9A-Z]+$/,
    DATE: /^\d{4}(-|\/|.)\d{1,2}\1\d{1,2}$/, // 支持2018-01-01 和2018-1-01,
    IDENTITY: /^(^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$)|(^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[Xx])$)$/ // 身份证
}
...
export default {
    ...
    isDate (text, isTrim) { return validate(text, this.REGEXP.DATE, isTrim) },
    isIdcard (text, isTrim) { return validate(text, this.REGEXP.IDCARD, isTrim) },
    isUserName (text, isTrim) { return validate(text, this.USER_NAME, isTrim) },
